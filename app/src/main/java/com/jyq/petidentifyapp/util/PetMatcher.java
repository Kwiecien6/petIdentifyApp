...
public class PetMatcher {
    ...
    public int matchImageForVideo(Bitmap bitmap) {
        if (counter < MAX_COUNTER) {
            // 初始化待匹配图像
            Mat tempMat = new Mat();
            Utils.bitmapToMat(bitmap, tempMat);
            tempMat.convertTo(tempMat, CvType.CV_32F);
            for (int i = 0; i < mPathList.size(); i++) {
                String path = mPathList.get(i);
    ...
    public int matchImageForPhoto(Bitmap bitmap) {
        // 初始化待匹配图像
        Mat tempMat = new Mat();
        Utils.bitmapToMat(bitmap, tempMat);
        tempMat.convertTo(tempMat, CvType.CV_32F);
...
//            if (Math.abs(dmatch.queryIdx - dmatch.trainIdx) < 1f) {
